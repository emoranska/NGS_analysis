...
# create column with list of samples with frequency < 0.3
...
te_filter = te_matrix.drop(columns=['less_0.3', 'eq_0', 'no_ins', 'ok_ins']).rename(columns={
...
# create final list of <=3 samples with and without MITE insertions
te_filter['no_te_ins'] = te_filter['no_te_ins'].astype(str)
te_filter['te_ins'] = te_filter['te_ins'].astype(str)
te_filter_final = (te_filter.loc[(te_filter['te_ins'] != '[]') & (te_filter['no_te_ins'] != '[]')])
